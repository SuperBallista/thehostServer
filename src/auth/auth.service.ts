...
export class AuthService {
  ...
  async handleRefreshToken(
    req: Request,
  ): Promise<{ token: string; user: { id: number; nickname: string } }> {
  ...
  async authCallbackFlow(
    code: string,
  ): Promise<{ url: string; refreshToken: string }> {
    ...
    const existingUser = await this.userService.findUserByOAuthId(
      userInfo.id,
      'google',
    );
    //계정 DB정보를 바탕으로 전달할 토큰과 닉네임 정보를 생성합니다
    const { userId, accessToken, refreshToken, tempToken, nickname } =
      await this.checkExistingAccount(existingUser, userInfo.id);
    // 만약 유저 정보가 있으면 캐싱합니다.
    if (existingUser) await this.userCacheService.setUser(userId, existingUser);
    //생성된 정보를 바탕으로 토큰 URL 쿼리를 생성합니다
    const url = await this.makeUriData(
      accessToken,
      tempToken,
      nickname,
      userId,
  ...
  async checkExistingAccount(userData: UserDto | null, oauthId: string) {
    if (userData) {
      ...
      const tempToken = await this.jwtService.generateTempToken(
        oauthId,
        'google',
      );
  ...
  private async makeUriData(accessToken, tempToken, nickname, userId) {
    ...
    if (tempToken) {
      ...
      throw new HttpException(
        '사용자 검색 처리 중 오류가 발생하였습니다',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
  }

  async createNewNickname(
    Information: changeNicknameInfo,
    user?: UserTypeDecorater,
  ) {
    ...
    if (Information.token) {
      const { provider, oauthId } = await this.checkTempToken(
        Information.token,
      );
      const userData = await this.userService.addNewAccount(
        oauthId,
        provider,
        fullNickname,
      );
  ...
  private async checkTempToken(token) {
    ...
    if (
      ...
      throw new HttpException(
        'Invalid temporary token payload',
