...
import {
  convertKoreanToItemCode,
  extractAndConvertItems,
  extractPlayerIdFromNickname,
  convertZombieControlParams,
...
export class ActionService {
  ...
  async executeAction(
    ...
    try {
      ...
      const convertedParams = this.convertKoreanParamsToCode(
        action.params || {},
      );
      ...
      if (action.action === 'useItem' || action.action === 'giveItem') {
        ...
        if (!item) {
          this.logger.warn(
            `봇 ${botId}: ${action.action} - 아이템이 지정되지 않음`,
          );
          return;
        }
        if (!playerData.items || !playerData.items.includes(item as ItemCode)) {
          this.logger.warn(
            `봇 ${botId}: ${action.action} - 보유하지 않은 아이템: ${item}`,
          );
          this.logger.warn(
            `현재 보유 아이템: ${playerData.items?.join(', ') || '없음'}`,
          );
          return;
        }
      }
      ...
      this.logger.log(
        `봇 ${botId}: 액션 실행 완료: ${action.action}`,
        convertedParams,
      );
    } catch (error) {
      this.logger.error(
        `봇 ${botId}: 액션 실행 실패: ${error.message}`,
        error.stack,
      );
    }
  }
   ...
   * 한글 파라미터를 코드로 변환
   */
  private convertKoreanParamsToCode(
    params: Record<string, unknown>,
    ...
    if (typeof converted.target === 'string') {
      const playerId = extractPlayerIdFromNickname(
        converted.target,
        ANIMAL_NICKNAMES,
      );
      if (playerId !== null) {
    ...
    if (typeof converted.receiver === 'string') {
      ...
      this.logger.debug(
        `receiver 변환: "${converted.receiver}" -> ${playerId}`,
      );
      if (playerId !== null) {
        converted.receiver = playerId;
      } else {
        this.logger.warn(
          `receiver 닉네임을 플레이어 ID로 변환 실패: "${converted.receiver}"`,
        );
      }
    }
    ...
    // 내용(content)도 한글 아이템명 유지 (사용자에게 보여주기 위해)
    // content 변환 제거 - 낙서, 무전기 메시지 등도 한글로 표시되어야 함
  ...
  private async handleUseItemAction(
    ...
    if (!playerData.items || !playerData.items.includes(item)) {
      this.logger.warn(
        `봇 ${botId}이(가) 보유하지 않은 아이템 사용 시도: ${item}`,
      );
      this.logger.warn(
        `현재 보유 아이템: ${playerData.items?.join(', ') || '없음'}`,
      );
  ...
  private async handleGiveItemAction(
    ...
    if (!playerData.items || !playerData.items.includes(item)) {
      this.logger.warn(
        `봇 ${botId}이(가) 보유하지 않은 아이템 전달 시도: ${item}`,
      );
      this.logger.warn(
        `현재 보유 아이템: ${playerData.items?.join(', ') || '없음'}`,
      );
    ...
    if (typeof receiver !== 'number') {
      this.logger.warn(
        `받는 사람이 올바르지 않음. receiver 타입: ${typeof receiver}, 값: ${receiver}`,
      );
      this.logger.warn(`전체 params: ${JSON.stringify(params)}`);
      throw new Error(
        `받는 사람이 올바르지 않음: ${receiver} (${typeof receiver})`,
      );
    }
  ...
  private async handleZombieControlAction(
    ...
    if (Array.isArray(params.zombies)) {
      ...
      if (firstZombie && typeof firstZombie.zombie === 'string') {
        // 닉네임 형식으로 전달된 경우 변환
        zombies = convertZombieControlParams(
          params.zombies as any[],
          ANIMAL_NICKNAMES,
        );
      } else {
        // 이미 ID 형식인 경우 (레거시 지원)
        zombies = params.zombies as Array<{
          playerId: number;
          targetId?: number;
          nextRegion?: number;
        }>;
    ...
    const botNickname =
      ANIMAL_NICKNAMES[playerData.playerId] || `Bot_${Math.abs(botId)}`;
    this.logger.log(
      `${botNickname}이(가) 좀비 ${zombies.length}마리에게 명령 전달`,
    );
   ...
   * 스프레이 사용
   */
  private async useSpray(
    gameId: string,
    playerData: GamePlayerInRedis,
    content: string,
  ): Promise<void> {
    // 실제 GameService의 handleUseItem을 통해 스프레이 사용 처리
    await this.gameService.handleUseItem(
      playerData.userId,
      {
        item: 'spray',
        content: content,
    ...
    // 동물 닉네임으로 로그 출력
    const botNickname =
      ANIMAL_NICKNAMES[playerData.playerId] ||
      `Bot_${Math.abs(playerData.userId)}`;
  ...
  private async useWireless(
    ...
    this.logger.log(
      `${botNickname}이(가) ${targetNickname}에게 무전기 메시지 전송: ${message}`,
    );
  }
  ...
  private async removeItemFromInventory(
    ...
    const playerData = await this.playerManagerService.getPlayerData(
      gameId,
      playerId,
    );
