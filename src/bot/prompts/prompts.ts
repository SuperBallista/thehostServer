...
- 도망 가능: ${context.canEscape ? '가능' : '불가능'}${context.zombieList && context.zombieList.length > 0 ? `
...
- 낙서스프레이: useItem { "item": "낙서스프레이", "content": "낙서 내용" }
...
- 위치: ${context.currentRegion}
- 보유 아이템: ${context.currentItems.join(', ') || '없음'}
- 같은 구역 플레이어: ${context.playersInRegion.join(', ') || '없음'}${context.zombieList && context.zombieList.length > 0 ? `
- 좀비 현황: ${context.zombieList.map(z => `${z.nickname}(${z.location})`).join(', ')}` : ''}
