...
export const KOREAN_TO_ITEM_CODE = Object.entries(ITEM_NAMES).reduce(
  (acc, [code, name]) => {
    if (name) {
      acc[name] = code;
    }
 ...
 */
export function extractAndConvertItems(
  text: string,
): { korean: string; code: string }[] {
 ...
 */
export function extractPlayerIdFromNickname(
  text: string,
...
export function convertZombieControlParams(
  ...
  return zombies
    .map((zombie) => {
      ...
      // 좀비 닉네임을 ID로 변환
      if (zombie.zombie) {
        const zombieId = extractPlayerIdFromNickname(
          zombie.zombie,
          animalNicknames,
        );
        if (zombieId !== null) {
      ...
      // 타겟 닉네임을 ID로 변환
      if (zombie.target) {
        const targetId = extractPlayerIdFromNickname(
          zombie.target,
          animalNicknames,
        );
        if (targetId !== null) {
