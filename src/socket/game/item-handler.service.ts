...
export class ItemHandlerService {
  ...
  async handleGiveItem(
    ...
    giveItem: { item: ItemInterface; receiver: number },
  ) {
    // 아이템 소유 확인
    ...
    // 받는 사람 데이터 가져오기
    const receiverData = await this.playerManagerService.getPlayerData(
      gameId,
      giveItem.receiver,
    );
    if (!receiverData) {
    ...
    if (playerData.regionId !== receiverData.regionId) {
      throw new Error(
        '같은 지역에 있는 플레이어에게만 아이템을 전달할 수 있습니다',
      );
    }
    ...
    const receiverNickname =
      ANIMAL_NICKNAMES[receiverData.playerId] ||
      `플레이어${receiverData.playerId}`;
  ...
  async handleSprayUse(
    ...
    await this.gameDataService.saveRegionData(
      gameId,
      playerData.regionId,
      regionData,
    );
  ...
  async handleEraserUse(
    ...
    if (
      targetMessage < 0 ||
      targetMessage >= regionData.regionMessageList.length
    ) {
   ...
   * 백신 재료 조합 처리
   */
  async handleVaccineMaterialUse(
    gameId: string,
    playerData: GamePlayerInRedis,
  ) {
    // 필요한 재료들
    const requiredMaterials: ItemInterface[] = [
      'vaccineMaterialA',
      'vaccineMaterialB',
      'vaccineMaterialC',
    ];
  ...
  async handleWirelessUse(
    ...
    const targetNickname =
      ANIMAL_NICKNAMES[targetPlayer] || `플레이어${targetPlayer}`;
