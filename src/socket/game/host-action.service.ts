...
export class HostActionService {
  ...
  private async processZombieCommands(gameId: string, zombieCommands: any[]): Promise<void> {
    ...
    for (const zombieCommand of zombieCommands) {
      try {
        // 좀비 상태 먼저 확인
        const zombieState = await this.zombieService.getZombieState(gameId, zombieCommand.playerId);
        if (!zombieState) {
          console.warn(`[HostAction] 좀비 ${zombieCommand.playerId}를 찾을 수 없습니다`);
          continue;
        }
        ...
        // 시스템 메시지 생성 (호스트에게만)
        const zombieNickname = ANIMAL_NICKNAMES[zombieCommand.playerId] || `좀비 #${zombieCommand.playerId}`;
        const messages: string[] = [];

        // 명령 타입 확인 (공격 명령인지 이동 명령인지)
        const isAttackCommand = zombieCommand.targetId !== zombieState.targetId;
        const isMoveCommand = zombieCommand.nextRegion !== zombieState.nextRegion;

        // 공격 대상 설정 메시지 (공격 명령일 때만)
        if (isAttackCommand) {
          if (zombieCommand.targetId !== null && zombieCommand.targetId !== undefined) {
            const targetNickname = ANIMAL_NICKNAMES[zombieCommand.targetId] || `플레이어 #${zombieCommand.targetId}`;
            messages.push(`${zombieNickname}가 ${targetNickname}을(를) 공격 대상으로 설정했습니다.`);
          } else {
        ...
        if (isMoveCommand) {
          ...
          messages.push(`${zombieNickname}가 다음 이동 지역을 ${nextRegionName}으로 변경했습니다. (${turnsUntilMove}턴 후 이동 예정)`);
        }

        // 메시지가 있으면 호스트에게 전송
        if (messages.length > 0) {
          const hostPlayer = await this.playerManagerService.getPlayerDataByUserId(gameId, 
            await this.gameDataService.getHostUserId(gameId));
          if (hostPlayer) {
            for (const message of messages) {
              await this.chatService.sendSystemMessage(gameId, message, hostPlayer.regionId);
            }
          }
        }

        console.log(`[HostAction] 좀비 ${zombieCommand.playerId} 명령 처리 완료:`, {
          targetId: zombieCommand.targetId,
          nextRegion: zombieCommand.nextRegion
        });
      } catch (error) {
        console.error(`[HostAction] 좀비 ${zombieCommand.playerId} 명령 처리 실패:`, error);
      }
    }
