...
export class CombatHandlerService {
  ...
  async handleShotgunUse(gameId: string, playerData: any, targetPlayer?: number): Promise<userDataResponse> {
    ...
    // 숙주에게 좀비가 사살됨을 알림
    await this.notifyHostOfZombieKill(gameId, targetData.playerId, targetNickname);
   ...
   */
  private async updateAllPlayersSurvivorList(gameId: string, killedPlayerId: number) {
    const allPlayers = await this.playerManagerService.getAllPlayersInGame(gameId);
    ...
    for (const player of allPlayers) {
      if (player.userId > 0 && player.state !== 'left') {
        ...
        if (targetSocket) {
          ...
          console.log(`생존자 리스트 업데이트 전송: userId=${player.userId}, zombieKilled=${killedPlayerId}`);
        }
      }
    }
  }
   ...
   * 숙주에게 좀비 사살 알림
   */
  private async notifyHostOfZombieKill(gameId: string, killedPlayerId: number, targetNickname: string) {
    ...
    if (gameData && gameData.hostId > 0) {
      ...
      if (hostPlayerData) {
        ...
        if (hostData) {
          // 사살된 좀비를 리스트에서 제거
          hostData.zombieList = hostData.zombieList.filter(z => z.playerId !== killedPlayerId);
