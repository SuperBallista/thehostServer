...
export class GameService {
  ...
  async gameStart(userId: number): Promise<userDataResponse> {
    const state: LocationState = await this.redisService.getAndParse(
    ...
    const gamePlayers = players.map(
      (player, index) =>
        new GamePlayer(
          index,
          player.id,
          Math.floor(Math.random() * regionNumber),
          index === hostPlayer,
          regionNumber,
        ),
    ); // 게임플레이어 객체 세팅
  ...
  private async createNewGameData(
    ...
    for (const player of players) {
      await this.redisService.stringifyAndSet(
        `${gameDataIndex}:player:${player.playerId}`,
        player.recordData(),
      );
    } // 플레이어 데이터 생성
  ...
  async subscribeGameStart(
    ...
    try {
      ...
      const playerDataResult = await this.loadAllPlayersWithRetry(
        roomId,
        userId,
      );
      ...
      console.log(
        `${roomId}방 게임 시작 - 유저 ${userId} (플레이어 ${playerDataResult.myPlayerData.playerId})`,
      );
  ...
  private async updateLocationState(
    userId: number,
    roomId: string,
  ): Promise<void> {
    const locationData: { state: State; roomId: string } = {
      state: 'game',
      roomId,
    };
    await this.redisService.stringifyAndSet(
  ...
  private async loadAllPlayersWithRetry(
    ...
    for (let retry = 0; retry < MAX_RETRIES && !myPlayerData; retry++) {
      if (retry > 0) {
        console.log(
          `유저 ${userId}의 데이터를 찾는 중... 재시도 ${retry}/${MAX_RETRIES}`,
        );
