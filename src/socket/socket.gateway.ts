...
export class SocketGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  afterInit(server: Server) {
    ...
    this.redisPubSubService.registerGameStartCallback(
      async (roomData: Room) => {
  ...
  async handleConnection(@ConnectedSocket() client: Socket) {
    try {
      const result =
        await this.connectionService.verifyAndTrackConnection(client);
  ...
  async handleRequest(
    ...
    try {
      ...
      if (
        ...
        // 기존 로직 처리
        if (data.createRoom) {
          response = await this.lobbyService.createRoom(
            client,
            data.createRoom,
          );
        }
        ...
        if (data.exitRoom && data.user.id) {
          // 현재 위치 확인
          const locationState = await this.connectionService.getLocationData(
            data.user.id,
          );
          console.log(
            `[exitRoom] userId: ${data.user.id}, 현재 위치: ${locationState.state}, roomId: ${locationState.roomId}`,
          );

          if (locationState.state === 'game') {
            // 게임 중이면 gameService의 exitGame 호출
            console.log(
              `[exitRoom] 게임 나가기 처리 시작 - userId: ${data.user.id}`,
            );
            ...
            console.log(
              `[exitRoom] 방 나가기 처리 시작 - userId: ${data.user.id}`,
            );
            response = await this.lobbyService.exitToLobby(client);
          } else {
            // 이미 로비에 있는 경우
            console.log(
              `[exitRoom] 이미 로비에 있음 - userId: ${data.user.id}`,
            );
        ...
        if (data.hostAct && data.user.id) {
          response = await this.gameService.handleHostAction(
            data.user.id,
            data.hostAct,
          );
        }
        ...
        if (data.myStatus && data.user.id) {
          response = await this.gameService.updatePlayerStatus(
            data.user.id,
            data.myStatus,
          );
        }
        ...
        if (data.chatMessage && data.user.id) {
          response = await this.gameService.handleChatMessage(
            data.user.id,
            data.chatMessage,
          );
        }
        ...
        if (data.giveItem && data.user.id && data.roomId) {
          console.log('아이템 전달 요청:', {
            userId: data.user.id,
            giveItem: data.giveItem,
            roomId: data.roomId,
          });
          response = await this.gameService.handleGiveItem(
            data.user.id,
            data.giveItem,
            data.roomId,
          );
        ...
        if (data.useItem && data.user.id && data.roomId) {
          console.log('아이템 사용 요청:', {
            userId: data.user.id,
            useItem: data.useItem,
            roomId: data.roomId,
          });
          response = await this.gameService.handleUseItem(
            data.user.id,
            data.useItem,
            data.roomId,
          );
